 ...
 *
 * Storage format: `{ "key": "pageId", ... }`. Backward compatible: if the file contains
 * the old array format `["key", ...]` it is migrated on load (pageIds become null).
 *
...
export class FileDedupStore implements DedupStore {
  /** key → pageId (null for legacy entries that predate page-ID storage) */
